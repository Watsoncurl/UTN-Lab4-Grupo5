          ...
          <div class="card-body">
            <c:choose>
              <c:when test="${modo eq 'editar'}">
                <form action="ServletEditarCuenta" method="post" class="row g-3 needs-validation" novalidate>
              </c:when>
              <c:otherwise>
                <div class="row g-3">
              </c:otherwise>
                  ...
                  <c:choose>
                    <c:when test="${modo eq 'editar'}">
                      <select class="form-select" name="idCliente" required>
                        ...
                        <!-- Aquí deberías cargar dinámicamente la lista de clientes -->
                        <option value="${cuenta.id_cliente}" selected>${cuenta.cliente}</option>
                      </select>
                      <div class="invalid-feedback">Seleccione un cliente</div>
                    </c:when>
                    <c:otherwise>
                      <input type="text" class="form-control bg-light" 
                             value="${cuenta.cliente}" readonly>
                    </c:otherwise>
                  </c:choose>
                </div>
              </div>
                  ...
                  <c:choose>
                    <c:when test="${modo eq 'editar'}">
                      <select class="form-select" name="tipoCuenta" required>
                        ...
                        <!-- Aquí deberías cargar dinámicamente los tipos de cuenta -->
                        <option value="${cuenta.tipo_cuenta}" selected>${cuenta.tipo_cuenta}</option>
                      </select>
                ...
                <c:choose>
                  <c:when test="${modo eq 'editar'}">
                    <select class="form-select" name="estado" required>
                      <option value="true" ${cuenta.estado ? 'selected' : ''}>Activa</option>
                      <option value="false" ${not cuenta.estado ? 'selected' : ''}>Inactiva</option>
                    </select>
                  </c:when>
                  <c:otherwise>
                    <input type="text" class="form-control bg-light" 
              ...
              <div class="col-12 text-end mt-4">
                <c:choose>
                  ...
                  <c:otherwise>
                    <a href="ListarCuentasServlet" class="btn btn-primary px-4 me-2">
                      <i class="bi bi-arrow-left me-2"></i>Volver
                    </a>
                    <c:if test="${cuenta.estado}">
                      <a href="ServletEditarCuenta?id=${cuenta.id_cuenta}&modo=editar" class="btn btn-warning px-4">
