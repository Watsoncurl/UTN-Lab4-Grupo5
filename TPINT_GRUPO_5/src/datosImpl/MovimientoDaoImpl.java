...
public class MovimientoDaoImpl implements MovimientoDao {
    ...
    public List<Movimientos> obtenerPorCuenta(String nroCuenta) {
        ...
        try (PreparedStatement ps = conexion.prepareStatement(sql)) {
            ...
            System.err.println("Error al obtener movimientos por n√∫mero de cuenta: " + e.getMessage());
            e.printStackTrace();
        }
    ...
    public List<Movimientos> ListarTodosFiltradosPaginado(MovimientosFiltros filtro, int offset, int registrosPorPagina) {
        ...
        sql.append("ORDER BY m.fecha DESC ");  // Ordenar antes de limitar

        // --- PaginaciÛn ---
        if (registrosPorPagina != Integer.MAX_VALUE) { //Si es distinto a MAX_VALUE, significa que se esta paginando.
        ...
        try (PreparedStatement ps = conexion.prepareStatement(sql.toString())) {
            ...
            System.err.println("Error al filtrar movimientos (paginado): " + e.getMessage());
            e.printStackTrace();
        }
    ...
    public int obtenerTotalMovimientosFiltrados(MovimientosFiltros filtro) {
        ...
        try (PreparedStatement ps = conexion.prepareStatement(sql.toString())) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    totalMovimientos = rs.getInt(1); // Obtener el valor del conteo
                }
            }
        } catch (SQLException e) {
            System.err.println("Error al obtener el total de movimientos filtrados: " + e.getMessage());
            e.printStackTrace();
        }
    ...
    public List<Movimientos> ListarTodosConDescripcion() {
        ...
        try (PreparedStatement ps = conexion.prepareStatement(sql);
            ...
            System.err.println("Error al listar movimientos con descripci√≥n: " + e.getMessage());
        }
        return listaMovimientos;
    }
